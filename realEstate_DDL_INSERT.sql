...
drop table register;
drop table townhouse;
drop table sf_house;
drop table condo;
drop table property_tax;
drop table assess_val;
drop table owns;
...
drop table individual_owner;
drop table served_owner cascade constraints;
drop table served_buyer cascade constraints;
drop table realtor cascade constraints;
drop table payment;
drop table post_system;
...
(
    down_pay int not null,
    mortgage int not null,
    sell_price int not null,
    PRIMARY KEY (down_pay, mortgage)
);
...
(
    ...
    name char(60) not null,
    PRIMARY KEY (oid),
    FOREIGN KEY (oid) references served_owner (oid)
        ON DELETE CASCADE
);
...
(
    pid int not null,
    titid int not null UNIQUE,
    aid int not null UNIQUE,
    list_price int not null,
    sq_footage int not null,
    year_built int not null,
    PRIMARY KEY (pid),
    FOREIGN KEY (titid) references title (titid)
...
(
    ...
    FOREIGN KEY (pid) references property (pid)
        ON DELETE CASCADE,
    FOREIGN KEY (down_pay, mortgage) references payment (down_pay, mortgage)
        ON DELETE SET NULL
);
...
(
    ...
    PRIMARY KEY (aid),
    FOREIGN KEY (pid) references property (pid),
    FOREIGN KEY (postcode) references post_system (postcode)
);
...
(
    ...
    pid int not null,
    PRIMARY KEY (oid, pid),
    FOREIGN KEY (oid) references served_owner,
    FOREIGN KEY (pid) references property (pid)
        -- ON DELETE SET NULL
);

CREATE TABLE assess_val
(
...
(
    pid int not null,
    floor int,
    mgmt_company char(60),
    PRIMARY KEY (pid),
    FOREIGN KEY (pid) references property (pid)
);

CREATE TABLE sf_house
(
    pid int not null,
    lot_size int,
    PRIMARY KEY (pid),
...
(
    psid int not null,
    lid int not null,
    registered_date char(30),
    PRIMARY KEY (psid, lid),
    FOREIGN KEY (psid) references title (titid),
...
(
    ...
    authorized_date char(30),
    PRIMARY KEY (psid, lid),
    FOREIGN KEY (psid) references title (titid),
    FOREIGN KEY (lid) references lawyer (lid),
    FOREIGN KEY (bid) references served_buyer (bid)
        ON DELETE CASCADE
);
